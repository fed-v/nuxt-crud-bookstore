					...
					>
						<DialogPanel class="w-full min-w-[360px] p-5 bg-white shadow-lg rounded-2xl dark:bg-slate-800">
							...
							<p class="mt-1 text-sm text-gray-500">
								{{ book._id ? $t('updateBookMessage') : $t('createBookMessage') }}
							</p>

							<form @submit="submitBook" class="mt-5">
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									<div class="col-span-full">
										<FormInput rules="required" :label="$t('title')" type="text" name="title" id="title" />
									</div>
								...
								<div class="flex items-center justify-end space-x-3 mt-7">
									...
									<button type="submit" class="btn">{{ book._id ? $t('update') : $t('create') }}</button>
								</div>
							</form>
						</DialogPanel>
					</TransitionChild>
